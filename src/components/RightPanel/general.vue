<template>
  <el-form-item v-if="props.data.vModel !== undefined" v-show="false" label="字段名">
  ...
  <el-form-item label="默认值"
    v-if="props.data.layout !== 'rowFormItem' && props.data.layout !== undefined && !props.data.customDefaultValue">
    <el-input v-if="props.data.tag === 'my-text'" v-model="props.data.defaultValue" placeholder="请输入文本内容" />
    <component v-else :is="props.data.tag" v-model="props.data.defaultValue" style="width:100%" v-bind="props.data">
    </component>
  </el-form-item>
  ...
  <el-form-item v-if="props.data.border !== undefined && props.data.optionType === 'default'" label="是否带边框">
  ...
  <el-form-item v-if="props.data.show !== undefined" label="是否显示">
  ...
  <el-form-item v-if="props.data.required !== undefined" label="是否必填">
    <el-switch v-model="props.data.required" @change="requireChange" />
  ...
  <!-- <template v-if="props.data.layout === 'colFormItem'">
    ...
    <div v-for="(item, index) in props.data.regList" :key="index" class="reg-item">
